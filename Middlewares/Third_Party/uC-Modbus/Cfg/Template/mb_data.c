                        ...
                        CPU_INT16U  *perr)
{
	...
	*perr = MODBUS_ERR_NONE;
	...
	if( ( coil >= REG_COILS_START ) &&( coil < REG_COILS_START + REG_COILS_NREGS ) )
	{
		coil_val = usSCoilBuf[coil - REG_COILS_START];
	}
	else
	{
                 ...
                 CPU_INT16U   *perr)
{
	*perr = MODBUS_ERR_NONE;
	...
	if( ( coil >= REG_COILS_START ) &&( coil < REG_COILS_START + REG_COILS_NREGS ) )
	{
		usSCoilBuf[coil - REG_COILS_START] = coil_val;
	}
	else
	{
		*perr = MODBUS_ERR_RANGE;
                      ...
                      CPU_INT16U  *perr)
{
	...
	*perr = MODBUS_ERR_NONE;
	...
	if( ( di >= REG_DISCRETE_START ) &&( di < REG_COILS_START + REG_DISCRETE_NREGS ) )
	{
		di_val = usSCoilBuf[di - REG_DISCRETE_START];
	}
	else
	{
		*perr = MODBUS_ERR_RANGE;
                        ...
                        CPU_INT16U  *perr)
{
	...
	*perr = MODBUS_ERR_NONE;
	...
	if( ( reg >= REG_INPUT_START )&& ( reg < REG_INPUT_START + REG_INPUT_NREGS ) )
	{
		val = usSInBuf[reg - REG_INPUT_START];
	}
	else
	{
		*perr = MODBUS_ERR_RANGE;
                             ...
                             CPU_INT16U  *perr)
{
	...
	*perr = MODBUS_ERR_NONE;
	...
	if( ( reg >= REG_HOLDING_START )&& ( reg < REG_HOLDING_START + REG_HOLDING_NREGS ) )
	{
		val = usSHoldBuf[reg - REG_HOLDING_START];
	}
	else
	{
		*perr = MODBUS_ERR_RANGE;
                       ...
                       CPU_INT16U  *perr)
{
	*perr = MODBUS_ERR_NONE;
	...
	if( ( reg >= REG_HOLDING_START )&& ( reg < REG_HOLDING_START + REG_HOLDING_NREGS ) )
	{
		usSHoldBuf[reg - REG_HOLDING_START] = reg_val_16;
	}
	else
	{
		*perr = MODBUS_ERR_RANGE;
