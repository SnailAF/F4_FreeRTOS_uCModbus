...
#define PR2PRESCALER_DIV(PR_BITS) (4UL << (PR_BITS))
...
{
    const uint8_t pr = pick_min_iwdg_pr(timeout_ms);
    if (pr == INVALID_IWDG_PR) {
        return WATCHDOG_STATUS_INVALID_ARGUMENT;
    }
    const uint32_t rl = PR_TIMEOUT_MS2RL(pr, timeout_ms);
